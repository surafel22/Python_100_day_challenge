...
class QuizInterface:
    def __init__(self,quiz_brain: QuizBrain):
        ...
        self.correct = Button(image=correct_image, command=self.true_pressed)
        self.correct.grid(row=2,column=0)

        wrong_image = PhotoImage(file="images/false.png")
        self.wrong = Button(image=wrong_image, command=self.false_pressed)
    ...
    def get_next_question(self):
        ...
        if self.quiz.still_has_questions():
            self.score_label.config(text=f"Score: {self.quiz.score} points")
